...
SEQ_LEN="${SEQ_LEN:-168}"
OUT="results/electricity/shanxi"
CACHE="$OUT/tsfm_cache_qmad_v1"
mkdir -p "$OUT" "$CACHE"

common=(
  --device "$DEVICE" --model_name chronos2-lgbm --seed "${SEED:-42}"
  --data_path data/SHANXI/shanxi_aligned.csv
  --regress_cols_path configs/columns/shanxi/realtime/regress_realtime_safe.json
  --tsfm_cols_path configs/columns/shanxi/realtime/tsfm_realtime_safe.json
  --data_split_path configs/time/data_split_SHANXI/realtime.json
  --tag shanxi_chronos2_h${PRED_LEN}_qmad_v1 --save_dir "$OUT/PLACEHOLDER"
